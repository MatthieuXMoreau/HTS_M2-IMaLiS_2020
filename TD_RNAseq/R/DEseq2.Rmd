...
The count matix stores for each samples (in columns) the number reads mapped to each ORF (in rows) counted with **bedtools multicov**
...
```{r warning=FALSE}
ggplot(res, aes(x = pvalue)) + geom_histogram(bins = 50)
```
...
nrow(FoldFc_inf2)
#write.table(FoldFc_inf2, row.names = T, quote = F, sep = "\t", file = "../results/RNAseq_FoldFc_inf2.txt")
```
